...
@interface GFBWebProgressLayerLoc ()
...
@implementation GFBWebProgressLayerLoc
...
- (void)initPath {
    ...
    [path addLineToPoint:CGPointMake(APPWIDTH, 2)];
    self.path = path.CGPath;
    self.strokeEnd = 0;
}

- (void)setProgressStyle:(WebProgressStyle)progressStyle {
    _progressStyle = progressStyle;
    if (progressStyle == webProgressStyle_Gradual) {
...
- (void)setProgressColor:(UIColor *)progressColor {
    ...
    if (_progressStyle == webProgressStyle_Normal) {
        self.strokeColor = progressColor.CGColor;
    } else {
        self.progressStyle = webProgressStyle_Gradual;
    }
}
...
- (void)progressChanged:(NSTimer *)timer {
    ...
    if (_progressStyle == webProgressStyle_Gradual) {
        _gradientLayer.locations = @[@(self.strokeEnd/2), @(self.strokeEnd)];
        _gradientLayer.frame = CGRectMake(0, 0, APPWIDTH*self.strokeEnd, 2);
    }
}
